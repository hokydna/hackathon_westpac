"""The synthesis prompt contract — FROZEN in the base commit.

Provenance: adopted byte-for-byte from session D's `training/ftdata/prompts.py`
(commit c7bcf84) and promoted to `src/` as the single frozen definition, because
**3,773 training records were already generated against these exact strings.**
Verified: all 390 val records match `SYNTH_SYSTEM` or `SENTIMENT_SYSTEM` exactly,
zero mismatches. `tests/test_base_commit.py` pins both by SHA-256 — if that pin
fails, either revert the change or regenerate every record and update the hash in
the same commit.

Imported by `src/agent/synth.py` (session B), `src/tools/registry.py` (session A,
Role 2) and `training/prepare_data.py` (session D). `training/ftdata/prompts.py`
should be reduced to a re-export from here so there is exactly one definition —
that is a one-line change on `feat/training` and belongs to session D.
...
#: In-band truncation marker. Length is load-bearing -- see format_evidence.
_TRUNC_SUFFIX = "\n... [evidence truncated]"
...
def format_evidence(
    ...
    if len(text) > char_cap:
        # Reserve exactly the suffix length. The original `char_cap - 24` was an
        # off-by-one: the suffix is 25 characters, so the result came back at
        # char_cap + 1. Harmless at 1200, but a function must not exceed its own
        # documented cap. 13 of the 3,773 generated records are 1 char over as a
        # result; that does not warrant regeneration.
        text = text[: char_cap - len(_TRUNC_SUFFIX)].rstrip() + _TRUNC_SUFFIX
    return text
